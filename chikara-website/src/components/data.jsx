export const socialIcons = [
  {
    icon: 'instagram',
    url: 'https://www.instagram.com/chikaraoficial/'
  },
  {
    icon: 'facebook',
    url: 'https://www.facebook.com/chikaraoficial/'
  },
  {
    icon: 'youtube',
    url: 'https://www.youtube.com/channel/UCG51dRdn45UfK58rMeqrCPw/'
  },
  {
    icon: 'twitch',
    url: 'https://www.twitch.tv/chikaraoficial/'
  }
]

export const infos = [
  {
    icon: 'temple',
    title: '¿Quiénes somos?',
    info: 'Somos Chikara 力, una organización que se dedica a compartir la cultura y el idioma japonés con jóvenes y cualquier persona interesada.'
  },
  {
    icon: 'mtfuji',
    title: 'Cultura',
    info: 'En Chikara 力 nos encargamos de promover la cultura japonesa a través de eventos, cursos, exposiciones, talleres, charlas y encuentros.'
  },
  {
    icon: 'sakura',
    title: 'Lengua japonesa',
    info: 'En Chikara 力 fomentamos el aprendizaje del idioma japonés enseñando sus fundamentos y promoviendo el autoaprendizaje.'
  }
]

export const infosYoutube = {
    icon: 'tengu',
    title: '¿Te gusta Japón? ¿Quieres aprender su idioma y su cultura?',
    info: 'Entonces este canal de youtube es para ti. Aquí encontrarás cursos de japonés, podcast y noticias relacionadas con Japón. Todo explicado de forma divertida y sencilla por profesores entusiastas. ¡Suscríbete y disfruta de este viaje con nosotros!',
    video: 'https://www.youtube.com/embed/phAFWfiEo14'
  } 

export const infosTwitch = {
    icon: 'ramen',
    title: '¿Te gustan los podcast y los videojuegos?',
    info: 'Entonces no te pierdas nuestro canal de Twitch, donde podrás ver charlas, conferencias, cursos, gameplays y reacciones a videos sobre Japón. Es una forma divertida y entretenida de acercarte al país del sol naciente. ¡Anímate y suscríbete!',
    stream: 'https://player.twitch.tv/?channel=chikaraoficial&parent=www.chikaraoficial.org',
    chat: 'https://www.twitch.tv/embed/chikaraoficial/chat?parent=www.chikaraoficial.org'
  }

export const comments = [
  {
    name: 'Ivon Peralta',
    image: 'certificateIvon',
    location: 'Funza, Colombia',
    flag: 'colombia',
    comment: 'Este curso fue una excelente introducción al idioma japonés. Los profesores eran muy amables y pacientes, y el material del curso estaba bien organizado.'
  },
  {
    name: 'Tatiana Lopez',
    image: 'certificateTatiana',
    location: 'Mosquera, Colombia',
    flag: 'colombia',
    comment: 'Aprendí mucho en este curso de japonés básico. Los ejercicios prácticos y las actividades en grupo fueron muy útiles para mejorar mis habilidades.'
  },
  {
    name: 'Yajaira Ibarra',
    image: 'certificateYajaira',
    location: 'México',
    flag: 'mexico',
    comment: 'Aprendí mucho sobre la cultura japonesa a través de este curso y me siento más conectado con el país'
  },
    
  {
    name: 'Sofia Pulecio',
    image: 'certificateSofia',
    location: 'Funza, Colombia',
    flag: 'colombia',
    comment: 'Me encantó este curso de japonés básico. Los profesores eran muy conocedores y el contenido del curso estaba bien estructurado. ¡Definitivamente lo recomendaría a cualquiera que quiera aprender japonés!'
  },
  {
    name: 'Anónimo',
    image: 'certificateAnonimo',
    location: 'México',
    flag: 'mexico',
    comment: 'Este curso de japonés básico superó mis expectativas. Aprendí mucho en un corto período de tiempo y me siento más segura hablando japonés ahora.'
  },
  {
    name: 'Rowina Elizondo',
    image: 'certificateRowina',
    location: 'Ciudad de México, México',
    flag: 'mexico',
    comment: 'El ambiente del curso era muy amigable y me sentí cómoda haciendo preguntas y participando en las discusiones.'
  },
  {
    name: 'Jesus Castellanos',
    image: 'certificateJesus',
    location: 'Mosquera, Colombia',
    flag: 'colombia',
    comment: 'Tender la oportunidad de enseñar lo mucho que aprendí en Chikara sobre la cultura japonesa a través de estos cursos me hizo sientir más conectado con el país.'
  },
  {
    name: 'Ricardo Mayorga',
    image: 'certificateRicardo',
    location: 'Mosquera, Colombia',
    flag: 'colombia',
    comment: 'Este curso fue una excelente oportunidad para introducir al idioma japonés personas con la motivación y ganas de aprender.'
  }
]

export const faqs = [
  {
    question: "¿Cómo puedo inscribirme en los cursos que ofrece Chikara?",
    answer: "Para inscribirte en uno de nuestros cursos, solo tienes que hacer clic en el botón de inscripción en la parte superior de la página. Después de inscribirte, te enviaremos información detallada sobre los cursos."
  },
  {
    question: "¿Los cursos de japonés tienen algún costo?",
    answer: "No, nuestros cursos son completamente gratuitos. No hay ningún costo asociado con ellos, ni antes, durante ni después del curso. Además, no se cobrará por el material brindado."
  },
  {
    question: "¿Puedo tomar los cursos si ya tengo conocimientos previos de japonés?",
    answer: "Sí, nuestros cursos están diseñados para adaptarse a diferentes niveles de conocimiento. Ofrecemos cursos básicos e introductorios para aquellos que están empezando a aprender japonés."
  },
  {
    question: "¿Con qué frecuencia se abren las inscripciones para los cursos?",
    answer: "Las inscripciones para nuestros cursos se abren semestralmente, a principios y mediados de cada año. Puedes consultar nuestro calendario para ver las fechas exactas."
  },
  {
    question: "¿En qué horario se imparten los cursos?",
    answer: "Nuestras clases se imparten entre las 18:00 y las 22:00 horas de Colombia (UTC-5)."
  },
  {
    question: "¿Recibiré una certificación al completar el curso?",
    answer: "Sí, al completar el curso recibirás una certificación por asistencia por parte de la organización Chikara. Además, te animamos a aplicar a los cursos virtuales de Japan Foundation a través de la plataforma MINATO para reforzar tus conocimientos y obtener una certificación adicional."
  },
  {
    question: "¿Cómo puedo apoyar a Chikara?",
    answer: "Si te gusta nuestro trabajo y quieres ayudarnos a llegar más lejos, puedes apoyarnos a través de nuestro KO-FI, solo tienes que hacer clic en el botón de donaciones en la parte superior de la página. Al hacerlo, tu nombre será incluido en nuestro Chikara Wall como muestra de agradecimiento y reconocimiento por tu apoyo."
  }
]

export const sponsors = [
  {
    name: 'Japan Foundation',
    url: 'https://www.jpf.go.jp/e/',
    logo: 'japanFoundation'
  },
  {
    name: 'JapanesePod101',
    url: 'https://www.youtube.com/@JapanesePod101',
    logo: 'jp101'
  },
  {
    name: 'Plataforma de Juventudes',
    url: 'https://www.instagram.com/plataformajuventudesmosquera/',
    logo: 'plataforma'
  },
  {
    name: 'Douso',
    url: 'https://douso.com.co/',
    logo: 'douso'
  }
]

export const members = [
  {
    photo: 'jesus',
    name: 'Jesus Andres Castellanos Aguilar',
    job: 'Co-Fundador / CEO / Ing. Sistemas'
  },
  {
    photo: 'daniel',
    name: 'Daniel David Mejia Vargas',
    job: 'Gerente de proyectos / Ing. Industrial'
  },
  {
    photo: 'tatiana',
    name: 'Angie Tatiana López Rodríguez',
    job: 'Directora de RRHH / Psicóloga'
  },
  {
    photo: 'david',
    name: 'David Velásquez Velásquez',
    job: 'Consultor de Soporte / Ing. Sistemas'
  },
  {
    photo: 'ricardo',
    name: 'Ricardo Mayorga Romero',
    job: 'Developer / Profesor / Ing. Electrónico'
  },
  {
    photo: 'camilo',
    name: 'Camilo Andres Clavijo Rincón',
    job: 'Especialista contenidos de diseño'
  },
  {
    photo: 'ana',
    name: 'Ana Maria Ibañez Cardozo',
    job: 'Profesora / Colaboradora'
  },
  {
    photo: 'smit',
    name: 'Jonathan Smit Martinez Muñoz',
    job: 'Admr. de Empresas / Colaborador'
  },
  {
    photo: 'ivon',
    name: 'Ivon Natalia Peralta Martinez',
    job: 'Estudiante / Colaboradora'
  }
]

export const flags = {
  argentina: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-argentina_1f1e6-1f1f7.png',
  bolivia: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-bolivia_1f1e7-1f1f4.png',
  canada: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-canada_1f1e8-1f1e6.png',
  chile: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-chile_1f1e8-1f1f1.png',
  colombia: 'https://em-content.zobj.net/thumbs/160/sony/336/flag-colombia_1f1e8-1f1f4.png',
  costarica: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-costa-rica_1f1e8-1f1f7.png',
  cuba: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-cuba_1f1e8-1f1fa.png',
  ecuador: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-ecuador_1f1ea-1f1e8.png',
  elsalvador: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-el-salvador_1f1f8-1f1fb.png',
  espana: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-spain_1f1ea-1f1f8.png',
  guatemala: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-guatemala_1f1ec-1f1f9.png',
  honduras: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-honduras_1f1ed-1f1f3.png',
  mexico: 'https://em-content.zobj.net/thumbs/160/sony/336/flag-mexico_1f1f2-1f1fd.png',
  nicaragua: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-nicaragua_1f1f3-1f1ee.png',
  panama: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-panama_1f1f5-1f1e6.png',
  paraguay: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-paraguay_1f1f5-1f1fe.png',
  peru: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-peru_1f1f5-1f1ea.png',
  puertorico: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-puerto-rico_1f1f5-1f1f7.png',
  repdominicana: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-dominican-republic_1f1e9-1f1f4.png',
  uruguay: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-uruguay_1f1fa-1f1fe.png',
  usa: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-united-states_1f1fa-1f1f8.png',
  venezuela: 'https://em-content.zobj.net/thumbs/120/sony/336/flag-venezuela_1f1fb-1f1ea.png'
}

/*
Función: Contiene los objetos y array de objetos con los texto de información de las secciones y componentes.

Este archivo esta hecho para modificar los texto de información sin entrar a los components ó Hooks.
*/